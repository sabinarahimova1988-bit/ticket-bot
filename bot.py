...
- name: имя пассажира латиницей, первая буква заглавная. Ищи поля "Пассажир", "ФИО пассажира", "Traveler", "Passenger", "Имя". Например: Ramazanov Elchin
- route: маршрут рейса. Ищи поля "Из/В", "From/To", "Маршрут", "Route", коды аэропортов (ALA=Алматы=Ala, BAK=Баку=Bak, IST=Стамбул=Ist, GYD=Баку=Gyd, TBS=Тбилиси=Tbs). Например: Ala-Bak или Bak-Ist-Bak
- date: дата ВЫЛЕТА рейса в формате YYYY-MM-DD. Ищи поля "Дата", "Date", "Вылет". НЕ брать дату выдачи билета или срок действия
- price: берётся из итоговой строки билета. Ищи строки: "TOTAL", "Total Amount", "ИТОГО", "Итого", "К оплате", "KZT", "AZN", "EUR", "USD" с суммой. Например "TOTAL : AZN 164.70" -> price=164.70, currency=AZN. Или "KZT 6,874.00" -> price=6874.00, currency=KZT. НЕ использовать AIR FARE, EQUIV FARE PAID или тарифы — только финальную итоговую сумму.
- currency: валюта итоговой суммы — AZN, EUR, USD, RUB или KZT. По умолчанию AZN.
...
Если пишет "добавь X к цене", "прибавь X", "добавь X azn к цене" — это action:price_add, НЕ update
...
=== ДОБАВЛЕНИЕ К ЦЕНЕ ===
Если пользователь пишет "добавь X к цене", "прибавь X azn", "добавь X к цене Ramazanov":
{"action":"price_add","name":"Имя Пассажира","route":"","amount":X}
...
- price: берётся из итоговой строки билета. Ищи строки: "TOTAL", "Total Amount", "ИТОГО", "Итого", "К оплате", "итого". Например "TOTAL : AZN 164.70" -> price=164.70, currency=AZN. Или "KZT 6,874.00" -> price=6874.00, currency=KZT. НЕ использовать AIR FARE, EQUIV FARE PAID или тарифы — только финальную итоговую сумму.
- currency: валюта итоговой суммы — AZN, EUR, USD, RUB или KZT. По умолчанию AZN.
...
def extract_name_route_from_bot_message(text):
    """Извлекает имя и маршрут из предыдущего сообщения бота."""
    import re
    name = ""
    route = ""
    # Ищем строку с именем: 👤 Имя или *Имя* · Маршрут
    name_match = re.search(r'👤 ([A-Za-z ]+)', text)
    if name_match:
        name = name_match.group(1).strip()
    # Ищем маршрут после имени: ✈️ Bak-Tbs или *Имя* · Bak-Tbs
    route_match = re.search(r'✈️ ([A-Za-z-]+)', text)
    if route_match:
        route = route_match.group(1).strip()
    if not route:
        route_match2 = re.search(r'\*[^*]+\* · ([A-Za-z-]+)', text)
        if route_match2:
            route = route_match2.group(1).strip()
    return name, route
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Если это reply на сообщение бота — извлекаем имя/маршрут и добавляем в текст
    replied = update.message.reply_to_message
    if replied and replied.from_user and replied.from_user.is_bot:
        name, route = extract_name_route_from_bot_message(replied.text or "")
        if name:
...
async def process_result(update: Update, context: ContextTypes.DEFAULT_TYPE, result: dict):
    ...
    if action == "price_add":
        ...
        route = result.get("route", "")
        amount = float(result.get("amount", 0))
        indices = find_tickets(tickets, name=name, route=route if route else None)
        if not indices:
            await update.message.reply_text("❌ Билет не найден. Проверьте имя или маршрут.")
            return
        if not amount:
            await update.message.reply_text("❌ Не указана сумма для добавления.")
            return
        for i in indices:
            old_price = float(tickets[i].get("price_azn", tickets[i].get("price", 0)))
            ...
            cu = float(tickets[i].get("cu", 0))
            ca = float(tickets[i].get("ca", 0))
            tickets[i]["owesUs"] = new_price + cu + ca
            tickets[i]["owesAgent"] = new_price + ca
        save_tickets(tickets)
        lines = [f"✏️ Цена обновлена (+{money(amount)} AZN):"]
        for i in indices:
            t = tickets[i]
            lines.append(f"• {t['name']} · {t['route']} · {fmt_date(t['date'])}\n"
                        f"  💰 Новая цена: {money(t['price_azn'])} AZN\n"
                        f"  🟢 Нам: {money(t['owesUs'])} AZN · 🟡 Агенту: {money(t['owesAgent'])} AZN")
        await update.message.reply_text("\n".join(lines))
        return
